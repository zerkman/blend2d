...
#include "blend2d_vpo.h"
#include "blend2d_fpo.h"
...
void init_screen(displayData *vdat) {
  ...
  /* Initialize the shader data */
  vdat->vpo = (rsxVertexProgram*)blend2d_vpo;
  vdat->vp_ucode = rsxVertexProgramGetUCode(vdat->vpo);
  vdat->fpo = (rsxFragmentProgram*)blend2d_fpo;
  vdat->fp_ucode = rsxFragmentProgramGetUCode(vdat->fpo, &fpsize);
  vdat->fp_buffer = (u32*)rsxMemalign(64,fpsize);
...
void blend2d(displayData *vdat, Bitmap *bitmap,
  ...
  texture.pitch = bitmap->width * 4;
  texture.offset = bitmap->offset;

  rsxLoadTexture(vdat->context,vdat->id_texture,&texture);
  rsxTextureControl(vdat->context,vdat->id_texture,GCM_TRUE,0<<8,12<<8,GCM_TEXTURE_MAX_ANISO_1);
